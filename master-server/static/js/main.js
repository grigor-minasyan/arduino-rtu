...
function update_temp() {
  $.getJSON($SCRIPT_ROOT + '/_update_cur_temp/' + RTU_id.toString(), function(data) {
    // $("#cur_temp").text(data.cur_temp);
    // $("#cur_hum").text(data.cur_hum);
    RTU_obj = data;
    RTU_obj[3].sort(function(item2, item1){
    ...
    str = "";
    for (let i = 0; i < Object.keys(RTU_obj[3]).length; i++) {
      str = str + "<tr><td>" +
      RTU_obj[3][i][0].toString() + "/" +
      RTU_obj[3][i][1].toString() + "/" +
      RTU_obj[3][i][2].toString() + "</td><td>" +
      RTU_obj[3][i][3].toString() + ":" +
      RTU_obj[3][i][4].toString() + ":" +
      RTU_obj[3][i][5].toString() + "</td><td>" +
      RTU_obj[3][i][6].toString() + "C " +
      RTU_obj[3][i][7].toString() + "%" +"</td></tr>";
    }
    $("#history").html(str);
  });
  return false;
}
window.onLoad = update_temp;
